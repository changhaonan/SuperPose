...
def merge_(anno_2d_file, avg_anno_3d_file, collect_anno_3d_file, idxs_file, img_id, ann_id, images, annotations):
...
def merge_anno(cfg):
    ...
    for anno_dir in anno_dirs:
        ...
        img_id, ann_id = merge_(anno_2d_file, avg_anno_3d_file, collect_anno_3d_file, idxs_file, img_id, ann_id, images, annotations)
...
def sfm(cfg):
    ...
    for data_dir in data_dirs:
        ...
        img_lists = []
        seg_lists = []
        img_lists += glob.glob(str(Path(data_dir)) + "/color/*.png", recursive=True)
        seg_lists += glob.glob(str(Path(data_dir)) + "/seg/*.png", recursive=True)
        ...
        postprocess(cfg, img_lists, data_dir, outputs_dir_root)


def sfm_core(cfg, img_lists, seg_lists, outputs_dir_root):
    """Sparse reconstruction: extract features, match features, triangulation"""
    from one_pose.sfm import extract_features_ext, match_features, generate_empty, triangulation, pairs_from_poses
