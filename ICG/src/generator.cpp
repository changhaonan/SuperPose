...
namespace icg
{
  ...
  {
    if (file_storage[class_name].empty())
      return true;
    ...
    for (const cv::FileNode &file_node : file_nodes)
    {
      ...
      if (!GeneratorFunction(file_node, &object_ptr))
        return false;
        ...
        [&](const auto &file_node, auto *feature_modality_ptr)
        {
          ...
          if (MetafilePathEmpty(file_node))
            *feature_modality_ptr = std::make_shared<FeatureModality>(
                Name(file_node), body_ptr, color_camera_ptr, depth_camera_ptr, feature_model_ptr);
          else
            *feature_modality_ptr = std::make_shared<FeatureModality>(
                Name(file_node), MetafilePath(file_node, configfile_path),
                body_ptr, color_camera_ptr, depth_camera_ptr, feature_model_ptr);
          return true;
        },
        modality_ptrs);
  }
    ...
    return ConfigureObjects<FeatureViewer>(
        ...
        [&](const auto &file_node, auto *viewer_ptr)
        {
          ...
          if (!GetObject(file_node, camera_parameter_name, class_name,
            ...
            return false;
          if (MetafilePathEmpty(file_node))
            *viewer_ptr = std::make_shared<FeatureViewer>(Name(file_node), camera_ptr, renderer_geometry_ptr);
          else
                                 ...
                                 std::shared_ptr<Tracker> *tracker_ptr)
  {
    ...
    cv::FileStorage fs;
    if (!OpenYamlFileStorage(configfile_path, &fs))
      return false;
    ...
    if (!ConfigureRegionModalities(
        ...
        !ConfigureFeatureModalities(configfile_path, fs, body_ptrs,
                                    color_camera_ptrs, depth_camera_ptrs, feature_model_ptrs,
                                    &modality_ptrs))
      return false;
