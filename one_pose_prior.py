    ...
    for eval_dir in eval_dirs:
        ...
        if video_mode == "video":
            ...
            while True:
                ret, frame = cap.read()
                if ret:
                    pose_pred_homo, num_inliers = one_pose_inference.inference(cfg, frame)
                    pose_pred_list.append(np.linalg.inv(pose_pred_homo))
        ...
        pose_pred_list = np.stack(pose_pred_list, axis=0)
        num_inliers_list = np.stack(num_inliers_list, axis=0)
        np.savez(f"{eval_dir}/kf_results.npz", cam_poses=pose_pred_list, num_inliers_list=num_inliers_list)
